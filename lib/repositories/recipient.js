...
exports.find = (callback) => {
  database.connect((error, db) => {
    ...
    db.collection('recipients').find().toArray((error, recipients) => {
      if (error) {
        return callback(error)
      }
...
exports.findById = (id, callback) => {
  database.connect((error, db) => {
    ...
    db.collection('recipients').findOne({_id: new ObjectID(id)}, (error, recipient) => {
      if (error) {
        return callback(error)
      }
...
exports.delete = (id, callback) => {
  database.connect((error, db) => {
    ...
    db.collection('recipients').deleteOne({_id: new ObjectID(id)}, (error, result) => {
      if (error) {
        return callback(error)
      }
